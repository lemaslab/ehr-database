...
newdata=rename(abx, 
               part_id = `Baby-Id`, 
               abx_medication_order=`Med Order #`, 
               abx_medication_code=Med_Code, 
               abx_medication_name.raw=Medication, 
               abx_medication_date=`Med Order Datetime`)

## FORMAT FOR REDCAP

data_ready=newdata %>%
  mutate(redcap_repeat_instrument="antibiotics_rx",
         redcap_event_name=paste0(year(abx_medication_date),"_arm_1")) %>% 
  mutate(abx_medication_name=gsub(" ","_",abx_medication_name.raw)) %>%
  mutate(abx_medication_name=gsub(",","&",abx_medication_name)) %>%
  group_by(part_id,redcap_event_name) %>% mutate(redcap_repeat_instance = row_number()) %>%
  select("part_id","redcap_repeat_instrument","redcap_repeat_instance","redcap_event_name",everything()) %>%
  select(-abx_medication_name.raw) 
