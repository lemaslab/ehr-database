...
source(file.path(working_dir, "code", "functions", "process_mom_baby_link_simple.R"))

library(dplyr)

message("=== RUNNING MOM BABY LINK (SIMPLE) ===")

gnv <- process_mom_baby_link_simple("GNV", working_dir)
jax <- process_mom_baby_link_simple("JAX", working_dir)
