process_mom_baby_link_simple <- function(site, working_dir = getwd()) {
  ...
  if (site == "GNV") {
    ...
    birth_path   <- "V:/FACULTY/DJLEMAS/EHR_Data_raw/raw/READ_ONLY_DATASETS/10year_data/2025/dataset_04_2025/baby_mom_at_birth_with_payer_Jax.csv"
  } else {
  ...
  birth <- read_csv(birth_path, show_col_types = FALSE) %>%
    rename(
      part_id_infant_tmp = dplyr::any_of(c("Deidentified_baby_ID", "deidentified_baby_id")),
      part_dob_infant_raw = dplyr::any_of(c("Deid-Delivery Datetime", "date_of_delivery"))
    ) %>%
    mutate(
      part_id_infant = paste0(site, "-infant-", part_id_infant_tmp),
      part_dob_infant = suppressWarnings(mdy_hm(part_dob_infant_raw)),
      part_dob_infant = if_else(is.na(part_dob_infant),
                                as.POSIXct(mdy(part_dob_infant_raw)),
                                part_dob_infant)
    ) %>%
  ...
  df <- linkage %>%
    ...
    mutate(
      ...
      delivery_id = paste0(site, "-delivery-", delivery_num),
      site = site
    ) %>%
    ungroup() %>%
  ...
  save(df, file = file.path(out_dir, "mom_baby_link.rda"))
  readr::write_csv(df, file.path(out_dir, "mom_baby_link.csv"), na = "")
