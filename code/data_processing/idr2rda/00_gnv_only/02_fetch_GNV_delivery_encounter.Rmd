---
title: "fetch_data: delivery encounter data (October 2025; Gainesville)"
...
# **************************************************************************** #
#    GAINESVILLE:         baby_mom_at_birth_with_payer.csv                         
# **************************************************************************** #
...
# # Define gestational age parsing function (e.g., "37w5d")
# parse_gestational_age <- function(ga_string) {
#   matches <- str_match(ga_string, "(\\d+)w(\\d+)d")
#   weeks <- as.numeric(matches[, 2])
#   days <- as.numeric(matches[, 3])
#   list(weeks = weeks, days = days)
# }
...
# Convert to character before passing to compute_sfga
  ...
  mutate(part_id_mom=paste0("GNV-",part_id_mom),
         ...
         insurance_group = case_when(
            payer_mom %in% c("Medicaid", "Medicaid Managed Care", "Medicaid HMO", 
                       "Medicare", "Medicare HMO", "CMS", "Federal") ~ "Federal",
            payer_mom %in% c("Blue Cross", "Commercial", "Private") ~ "Private",
            payer_mom %in% c("Other") ~ "Other",
            payer_mom %in% c("SELF PAY") ~ "Self-Pay",
                                  TRUE ~ "Unknown"))
...
# ----------------------------------------------------------------------- #
delivery_encounter_tmp2 =left_join(
  delivery_encounter_tmp, 
  mom_baby_link_tmp, 
  by = c("part_id_infant")
  ) %>%
...
# rename
GNV_delivery_encounter_10_2025=delivery_encounter_tmp2
```
...
## EXPORT 
file_name="GNV_delivery_encounter"
...
save(GNV_delivery_encounter_10_2025, file = shared_path)

# Save locally
save(GNV_delivery_encounter_10_2025, file = local_path)
