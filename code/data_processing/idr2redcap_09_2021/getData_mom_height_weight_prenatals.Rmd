---
title: "IDR-to-Redcap: mom-height-weight-prenatals"
...
newdata=rename(data, 
               ...
               prenatal_encounter_type = `Encounter Type`,
               prenatal_encounter_date= `Height&Weight Datetime`,
               prenatal_wt_lbs = `Weight (lb)`,
               prenatal_ht_in = `Height (in)`
               )

# format dates
data_formatted = newdata %>%
  mutate(prenatal_encounter_date_tmp=as.POSIXct(prenatal_encounter_date, format="%m/%d/%Y %H:%M"),
         prenatal_encounter_date=format(strptime(prenatal_encounter_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-prenatal_encounter_date_tmp)

# FORMAT FOR REDCAP

data_ready=data_formatted %>%
  mutate(prenatal_encounter_date=ymd_hms(prenatal_encounter_date)) %>%
  mutate(redcap_repeat_instrument="anthropometrics",
         redcap_event_name=paste0(year(prenatal_encounter_date),"_arm_1"),
                                  year=year(prenatal_encounter_date),
         prenatal_wt_units="POUNDS",
         prenatal_wt_measure="WEIGHT",
         prenatal_ht_units="INCHES",
         prenatal_ht_measure="HEIGHT") %>%
  mutate(part_id=paste0("mom-",part_id)) %>%
  arrange(part_id,prenatal_encounter_date) %>%
...
data_file_name_export=paste0("mom_prenatal_hw_",as.character(data_ready[2,2]))
