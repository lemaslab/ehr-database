...
# read data
prenatal_hw=read_csv(data_import_path, col_types = cols()) 

# rename
# note: renaming to eliminate spaces EVEN THOUGH we will rename or drop later.
newdata=rename(prenatal_hw, 
               part_id = `Deidentified_mom_ID`, 
               encounter_type = `Encounter Type`,
               date_time= `Height&Weight Datetime`,
               prenatal_weight = `Weight (lb)`,
               prenatal_height = `Height (in)`
               )

# format dates
newdata = newdata %>%
  mutate(date_time_tmp=as.POSIXct(date_time, format="%m/%d/%Y %H:%M"),
         date_time=format(strptime(date_time_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-date_time_tmp)
...
data_ready=newdata %>%
  ...
  mutate(redcap_repeat_instrument="anthropometrics",
         redcap_event_name=paste0(year(date_time),"_arm_1"),
                                  year=year(date_time),
         prenatal_weight_units="POUNDS",
         prenatal_weight_measure="WEIGHT",
         prenatal_height_units="INCHES",
         prenatal_height_measure="HEIGHT") %>%
  mutate(part_id=paste0("mom-",part_id)) %>%
  arrange(part_id,date_time) %>%
...
data_file_name_export=paste0("mom_height_weight_prenatals_",as.character(data_ready[2,2]))
