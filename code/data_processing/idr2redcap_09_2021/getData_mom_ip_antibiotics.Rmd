---
title: 'IDR-to-RedCap: mom IP antbiotics'
...
# import variables
n_max=1000000
data_file_name="mom_IP_antibiotic.csv"
...
# read data
ip_antibiotics=read_csv(data_import_path, col_types = cols()) 

# rename
# note: renaming to eliminate spaces EVEN THOUGH we will rename or drop later.
newdata=rename(ip_antibiotics, 
               part_id = `Deidentified_mom_ID`, 
               med_name = `Med Order Display Name`,
               med_category = `Medication History Category`,
               med_status = `Med Status Category`,
               date_time_taken = `Taken Datetime`,
               med_therapy_class = `Med Therapy Class`,
               pharm_class = `Pharmacy Class`,
               pharm_subclass = `Pharmacy Subclass`,
               rxnorm_code = `Rxnorm Code`)

# format dates
newdata = newdata %>%
  mutate(date_time_taken_tmp=as.POSIXct(date_time_taken, format="%m/%d/%Y %H:%M"),
         date_time_taken=format(strptime(date_time_taken_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-date_time_taken_tmp)


# FORMAT FOR REDCAP

data_ready=newdata %>%
  mutate(redcap_repeat_instrument="ip_antibiotics",
         redcap_event_name=paste0(year(date_time_taken),"_arm_1"),
         year=year(date_time_taken)) %>%
  mutate(med_name=gsub(" ","_",med_name),
         med_category=gsub(" ","_",med_category),
         med_therapy_class=gsub(" ","_",med_therapy_class),
         pharm_class=gsub(" ","_",pharm_class),
         pharm_subclass=gsub(" ","_",pharm_subclass),
         part_id=paste0("mom-",part_id)) %>%
  arrange(part_id,date_time_taken) %>%
...
output_dir=paste0("Y:/DJLEMAS/EHR_Data/processed/redcap_ready/antibiotics/")
data_export_path=paste0(output_dir,data_file_name_export)

# Note: expecting to expot to PATH: ~/ehr-database/data/processed/redcap_ready/antibiotics/
