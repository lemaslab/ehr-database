---
title: "IDR-to-Redcap: baby-well-visit"
...
# import variables
n_max=1000000
data_file_name="baby_well_visit.csv"
...
newdata=rename(bmi, 
               ...
               wellvisit_wt_lbs=`Weight (lbs)`,
               wellvisit_ht_cm=`Height (cm)`, 
               wellvisit_headcir_cm=`Head Circumference (cm)`,
               wellvisit_date=`Observation Datetime`
               )

# format dates
bmi_ready = newdata %>%
  mutate(wellvisit_encounter_date_tmp=as.POSIXct(wellvisit_date, format="%m/%d/%Y %H:%M"),
         wellvisit_date=format(strptime(wellvisit_encounter_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-wellvisit_encounter_date_tmp)

# FORMAT FOR REDCAP

data_ready=bmi_ready %>%
  mutate(wellvisit_date=ymd_hms(wellvisit_date)) %>%
  mutate(redcap_repeat_instrument="anthropometrics",
         redcap_event_name=paste0(year(wellvisit_date),"_arm_1"),
                                  year=year(wellvisit_date),
         wellvisit_wt_units="POUNDS",
         wellvisit_wt_measure="WEIGHT",
         wellvisit_ht_units="CENTIMETERS",
         wellvisit_wt_measure="HEIGHT",
         wellvisit_headcir_units="CENTIMETERS",
         wellvisit_headcir_measure="HEAD_CIRCUMFERENCE") %>%
  mutate(part_id=paste0("baby-",part_id)) %>%
  arrange(part_id,wellvisit_date) %>%
...
data_file_name_export=paste0("baby_wellvisit_",as.character(data_ready[2,2]))
