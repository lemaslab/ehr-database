---
title: 'IDR-to-RedCap: baby ear infection codes'
...
# import data-01
data_file_name="baby_ear_infection.csv"
...
data=read_csv(data_import_path, col_types = cols()) %>% 
  rename(part_id = `Deidentified_baby_ID`, 
         ear_infection_dx_date = `Diagnosis Start Date`,
         ear_infection_dx_code = `Diagnosis Code`,
         ear_infection_dx_descrip = `Diagnosis Description`,
         ear_infection_dx_type = `Diagnosis Type`)
...
data_ready=data %>%
  mutate(redcap_repeat_instrument="diagnosis_codes",
         ear_infection_dx_code_type="EAR-INFECTION",
         redcap_event_name=paste0(year(ear_infection_dx_date),"_arm_1")) %>%
         mutate(ear_infection_dx_descrip=gsub(" ","_",ear_infection_dx_descrip)) %>%
  mutate(ear_infection_dx_type=gsub(" ","_",ear_infection_dx_type),
         part_id=paste0("baby-",part_id),
         year=year(ear_infection_dx_date)) %>%
  arrange(part_id,ear_infection_dx_date) %>%
...
data_file_name_export=paste0("baby_ear_infection_",as.character(data_ready[2,2]))
# output_dir=paste0("~/ehr-database/data/processed/redcap_ready/baby_ear_infection/")
output_dir=paste0("V:/FACULTY/DJLEMAS/EHR_Data/processed/redcap_ready/baby_ear_infection/")
