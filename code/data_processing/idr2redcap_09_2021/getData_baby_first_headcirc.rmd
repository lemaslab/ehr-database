---
title: "IDR-to-Redcap: baby-first-head-circumference"
...
# import variables
n_max=1000000
data_file_name="baby_first_head_circumference.csv"
...
newdata=rename(data, 
               ...
               first_headcirc_date=`1st Head Circumference Datetime`,
               first_headcirc_cm=`1st Head Circumference (cm)`
               )

# format dates
data_formatted = newdata %>%
  mutate(first_headcirc_date_tmp=as.POSIXct(first_headcirc_date, format="%m/%d/%Y %H:%M"),
         first_headcirc_date=format(strptime(first_headcirc_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-first_headcirc_date_tmp)

# FORMAT FOR REDCAP

data_ready=data_formatted %>%
  mutate(first_headcirc_date=ymd_hms(first_headcirc_date)) %>%
  mutate(redcap_repeat_instrument="anthropometrics",
         redcap_event_name=paste0(year(first_headcirc_date),"_arm_1"),
                                  year=year(first_headcirc_date),
         first_headcirc_units="CENTIMETERS",
         first_headcirc_measure="HEAD_CIRCUMFERENCE") %>%
  mutate(part_id=paste0("baby-",part_id)) %>%
  arrange(part_id,first_headcirc_date) %>%
...
data_file_name_export=paste0("baby_headcirc_",as.character(data_ready[2,2]))
