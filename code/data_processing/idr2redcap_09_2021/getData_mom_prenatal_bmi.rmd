...
newdata=rename(bmi, 
               ...
               prenatal_bmi=`Last Stamped BMI on encounter`, 
               encounter_date=`Last Stamped BMI on encounter datetime`
               )

# format dates
bmi_ready = newdata %>%
  mutate(encounter_date=mdy_ms(encounter_date),
         encounter_date=mdy(encounter_date))

# format dates
bmi_ready = newdata %>%
  mutate(encounter_date_tmp=as.POSIXct(encounter_date, format="%m/%d/%Y %H:%M"),
         encounter_date=format(strptime(encounter_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-encounter_date_tmp)
...
data_ready=bmi_ready %>%
  mutate(encounter_date=ymd_hms(encounter_date)) %>%
  mutate(redcap_repeat_instrument="anthropometrics",
         redcap_event_name=paste0(year(encounter_date),"_arm_1"),
         measure_units="kg/m2") %>%
  mutate(part_id=paste0("mom-",part_id)) %>%
  group_by(part_id,redcap_event_name,encounter_date) %>% mutate(redcap_repeat_instance = row_number()) %>%
