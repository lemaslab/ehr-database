...
# variables
# part_id, part_dob, delivery_mode, baby_admit_date, part_race
...
newdata=rename(bmi, 
               ...
               anthro_encounter_type=`Encounter Type`,
               anthro_value=`Last Stamped BMI on encounter`, 
               anthro_encounter_date=`Last Stamped BMI on encounter datetime`
               )

# format dates
bmi_ready = newdata %>%
  mutate(anthro_encounter_date_tmp=as.POSIXct(anthro_encounter_date, format="%m/%d/%Y %H:%M"),
         anthro_encounter_date=format(strptime(anthro_encounter_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-anthro_encounter_date_tmp)

# FORMAT FOR REDCAP

data_ready=bmi_ready %>%
  mutate(anthro_encounter_date=ymd_hms(anthro_encounter_date)) %>%
  mutate(redcap_repeat_instrument="anthropometrics",
         redcap_event_name=paste0(year(anthro_encounter_date),"_arm_1"),
         anthro_measure_units="kg/m2",
         anthro_measure="PRENATAL-BMI") %>%
  mutate(part_id=paste0("mom-",part_id)) %>%
  arrange(part_id,anthro_encounter_date) %>%
