---
title: 'IDR-to-RedCap: mom medications-OP'
...
data_dir=paste0("~/ehr-database/data/raw/dataset_09_2021/mom_medications/")
# data_dir=paste0("V:/FACULTY/DJLEMAS/EHR_Data/raw/dataset_09_2021/mom_medication/")
# data_dir=paste0("Y:/DJLEMAS/EHR_Data/raw/dataset_09_2021/mom_medication/")
...
data_formatted = newdata %>%
  mutate(med_op_date_tmp=as.POSIXct(med_op_date, format="%m/%d/%Y %H:%M"),
         med_op_date=format(strptime(med_op_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-med_op_date_tmp)


# FORMAT FOR REDCAP

data_ready=data_formatted %>%
  mutate(med_op_date=ymd_hms(med_op_date)) %>%
  mutate(redcap_repeat_instrument="medications",
         redcap_event_name=paste0(year(med_op_date),"_arm_1"),
         year=year(med_op_date),
         med_op_status="Out-Patient") %>%
         mutate(med_op_name=gsub(" ","_",med_op_name)) %>%
  mutate(med_op_name=gsub(",","&",med_op_name),
         part_id=paste0("mom-",part_id)) %>%
  arrange(part_id,med_op_date) %>%
...
# export variables
batchSize=15000; # number of rows in single output file
data_file_name_export=paste0("mom_meds_op_",as.character(data_ready[2,2]))
# output_dir=paste0("~/ehr-database/data/processed/redcap_ready/medications/")
output_dir=paste0("V:/FACULTY/DJLEMAS/EHR_Data/processed/redcap_ready/medications/")
# output_dir=paste0("Y:/DJLEMAS/EHR_Data/processed/redcap_ready/medications/")
