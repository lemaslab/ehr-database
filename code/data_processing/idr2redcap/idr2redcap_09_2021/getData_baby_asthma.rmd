---
title: 'IDR-to-RedCap: baby asthma codes'
...
# import data-01
data_file_name="baby_asthma.csv"
...
data=read_csv(data_import_path, col_types = cols()) %>% 
  rename(part_id = `Deidentified_baby_ID`, 
         asthma_dx_date = `Diagnosis Start Date`,
         asthma_dx_code = `Diagnosis Code`,
         asthma_dx_descrip = `Diagnosis Description`,
         asthma_dx_type = `Diagnosis Type`)
...
data_ready=data %>%
  mutate(redcap_repeat_instrument="diagnosis_codes",
         asthma_dx_code_type="ASTHMA",
         redcap_event_name=paste0(year(asthma_dx_date),"_arm_1")) %>%
         mutate(asthma_dx_descrip=gsub(" ","_",asthma_dx_descrip)) %>%
  mutate(asthma_dx_type=gsub(" ","_",asthma_dx_type),
         part_id=paste0("baby-",part_id),
         year=year(asthma_dx_date)) %>%
  arrange(part_id,asthma_dx_date) %>%
...
data_file_name_export=paste0("baby_asthma_",as.character(data_ready[2,2]))
# output_dir=paste0("~/ehr-database/data/processed/redcap_ready/baby_asthma/")
output_dir=paste0("V:/FACULTY/DJLEMAS/EHR_Data/processed/redcap_ready/baby_asthma/")
