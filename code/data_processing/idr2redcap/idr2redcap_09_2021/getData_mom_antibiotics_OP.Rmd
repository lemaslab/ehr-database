...
newdata=rename(data, 
               ...
               abx_op_type = `Medication History Category`,
               abx_op_date = `Med Order Datetime`,
               abx_op_name = `Med Order Display Name`,
               abx_op_therapy_class = `Med Therapy Class`,
               abx_op_pharm_class = `Pharmacy Class`,
               abx_op_pharm_subclass = `Pharmacy Subclass`,
               abx_op_rxnorm_code = `Rxnorm Code`)

# format dates
data_formatted = newdata %>%
  mutate(abx_op_date_tmp=as.POSIXct(abx_op_date, format="%m/%d/%Y %H:%M"),
         abx_op_date=format(strptime(abx_op_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-abx_op_date_tmp)


# FORMAT FOR REDCAP
data_ready=data_formatted %>%
  mutate(abx_op_date=ymd_hms(abx_op_date)) %>%
  mutate(redcap_repeat_instrument="antibiotics",
         redcap_event_name=paste0(year(abx_op_date),"_arm_1"),
         year=year(abx_op_date),
         abx_op_status= "Out-Patient") %>%
         mutate(abx_op_name=gsub(" ","_",abx_op_name)) %>%
  mutate(abx_op_name=gsub(",","&",abx_op_name),
         part_id=paste0("mom-",part_id)) %>%
  arrange(part_id,abx_op_date) %>%
...
data_file_name_export=paste0("mom_abx_op_",as.character(data_ready[2,2]))
