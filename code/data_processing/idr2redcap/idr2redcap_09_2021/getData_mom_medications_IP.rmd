---
title: 'IDR-to-RedCap: mom medications-IP'
...
data_formatted = newdata %>%
  mutate(med_ip_date_tmp=as.POSIXct(med_ip_date, format="%m/%d/%Y %H:%M"),
         med_ip_date=format(strptime(med_ip_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-med_ip_date_tmp)


# FORMAT FOR REDCAP

data_ready=data_formatted %>%
  mutate(med_ip_date=ymd_hms(med_ip_date)) %>%
  mutate(redcap_repeat_instrument="medications",
         redcap_event_name=paste0(year(med_ip_date),"_arm_1"),
         year=year(med_ip_date)) %>%
         mutate(med_ip_name=gsub(" ","_",med_ip_name)) %>%
  mutate(med_ip_name=gsub(",","&",med_ip_name),
         part_id=paste0("mom-",part_id)) %>%
  arrange(part_id,med_ip_date) %>%
...
data_file_name_export=paste0("mom_meds_ip_",as.character(data_ready[2,2]))
