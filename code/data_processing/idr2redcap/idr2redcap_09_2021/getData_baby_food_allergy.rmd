---
title: 'IDR-to-RedCap: baby food allergy codes'
...
# import data-01
data_file_name="baby_food_allergy.csv"
...
data=read_csv(data_import_path, col_types = cols()) %>% 
  rename(part_id = `Deidentified_baby_ID`, 
         foodallergy_dx_date = `Diagnosis Start Date`,
         foodallergy_dx_code = `Diagnosis Code`,
         foodallergy_dx_descrip = `Diagnosis Description`,
         foodallergy_dx_type = `Diagnosis Type`)
...
data_ready=data %>%
  mutate(redcap_repeat_instrument="diagnosis_codes",
         foodallergy_dx_code_type="FOOD_ALLERGY",
         redcap_event_name=paste0(year(foodallergy_dx_date),"_arm_1")) %>%
         mutate(foodallergy_dx_descrip=gsub(" ","_",foodallergy_dx_descrip)) %>%
  mutate(foodallergy_dx_type=gsub(" ","_",foodallergy_dx_type),
         part_id=paste0("baby-",part_id),
         year=year(foodallergy_dx_date)) %>%
  arrange(part_id,foodallergy_dx_date) %>%
...
data_file_name_export=paste0("baby_foodallergy_",as.character(data_ready[2,2]))
# output_dir=paste0("~/ehr-database/data/processed/redcap_ready/baby_foodallergy/")
output_dir=paste0("V:/FACULTY/DJLEMAS/EHR_Data/processed/redcap_ready/baby_foodallergy/")
