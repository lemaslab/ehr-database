---
title: "IDR-to-Redcap: baby-first-height"
...
# import variables
n_max=1000000
data_file_name="baby_first_height.csv"
...
newdata=rename(data, 
               ...
               first_height_date=`1st height Datetime`,
               first_height_cm=`1st height(cm)`
               )

# format dates
data_formatted = newdata %>%
  mutate(first_height_date_tmp=as.POSIXct(first_height_date, format="%m/%d/%Y %H:%M"),
         first_height_date=format(strptime(first_height_date_tmp, 
                                             format='%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %I:%M:%S %p')) %>%  select(-first_height_date_tmp)

# FORMAT FOR REDCAP

data_ready=data_formatted %>%
  mutate(first_height_date=ymd_hms(first_height_date)) %>%
  mutate(redcap_repeat_instrument="anthropometrics",
         redcap_event_name=paste0(year(first_height_date),"_arm_1"),
                                  year=year(first_height_date),
         first_height_units="CENTIMETERS",
         first_height_measure="HEIGHT") %>%
  mutate(part_id=paste0("baby-",part_id)) %>%
  arrange(part_id,first_height_date) %>%
...
data_file_name_export=paste0("baby_height_",as.character(data_ready[2,2]))
